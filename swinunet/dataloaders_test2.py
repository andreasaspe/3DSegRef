...
def get_preprocessing_transforms(pixdim=(1.5, 1.5, 2.0)):
    """
...
def create_dataloaders_with_cache(
    ...
    """
    Create dataloaders using CacheDataset - preprocesses once, caches in RAM.
    RECOMMENDED: For datasets that fit in RAM (up to ~50GB).
    ...
    # Preprocessing transforms (cached)
    preprocess_transforms = get_preprocessing_transforms(pixdim=pixdim)
    
    # Augmentation transforms (applied after cache)
    augmentation_transforms = get_augmentation_transforms()
    
    # Create datasets with caching
    # Preprocessing is done once and cached, augmentation applied on-the-fly
    train_ds = CacheDataset(
        data=train_files,
        transform=preprocess_transforms,
        cache_rate=1.0,  # Cache all preprocessed data
        num_workers=num_workers
    )
    
    # Wrap cached dataset with augmentation
...
if __name__ == "__main__":
    ...
    # OPTION 1: Use CacheDataset (for datasets that fit in RAM)
    print("=" * 50)
    print("Option 1: CacheDataset (stores in RAM)")
    print("=" * 50)
